...
## 2026-08-15 - Milestone 12 signed compliance exports

- Intent: Generate independently verifiable integrity evidence that binds normalized compliance-report criteria to a captured global-chain proof.
- AI contribution: Proposed and implemented a separate compatibility-safe compliance manifest, criteria hashing, full/bridge selection, shared legacy/compliance chain verification, generalized Ed25519 signing with binary-compatible legacy overloads, export authority, privacy-minimized creation receipts, Swagger/Bruno guidance, tests, and an ADR.
- Engineer decision: Reviewed and explicitly approved for commit and push on 2026-08-15.
- Modifications after validation: Extracted common record verification instead of duplicating cryptographic logic. Restored the original typed signer overload after a regression test exposed JVM binary-method compatibility even though source compilation succeeded.
- Design boundary: Verification proves trusted signature, criteria integrity, disclosed event commitments, chain continuity, full-record membership, count, and head. Privacy-preserving bridges prevent independent re-evaluation of selection completeness, which remains a signer attestation. Asynchronous delivery and source completeness remain deferred.
- Validation: Focused compliance tests cover authority separation, normalized manifest criteria, bridge privacy, receipt minimization, valid independent verification, unsigned tampering, resigned criteria-hash tampering, and resigned scope tampering. The unchanged legacy export suite passes after shared-verifier extraction. The full Maven build passes with 39 tests, 94.47% line coverage, and 76.32% branch coverage.

## 2026-08-15 - Milestone 13 assignment readiness

- Intent: Audit the complete repository against the four-page source assignment, resolve confirmed gaps, and prepare an evidence-based handoff without adding speculative product scope.
- AI contribution: Re-read and visually checked every assignment page; built a requirement-to-code/test traceability matrix; reviewed architecture, API mappings, security, OpenAPI, CI, Docker, monitoring, submission documents, commit history, and repository privacy; completed the final engineering summary; and aligned stale documentation with the implemented system.
- Engineer decision: Milestone implementation was explicitly approved on 2026-08-15. Commit and push remain pending the engineer's separate approval.
- Accepted: Keep the modular monolith, global sequence chain, keyset pagination, soft archival, commitment redaction, full/bridge signed exports, controlled compliance schema, H2 default, and PostgreSQL setup-only boundary because each is traceable to an assignment need or stated engineer constraint.
- Modified: A clean GitHub Actions build exposed a missing `AuditExportRecord` import left by the Milestone 12 shared-verifier refactor. The import was restored. Redaction receipt attribution was also changed from caller-supplied `actorId` to the authenticated administrator, and body identity overrides are rejected.
- Rejected or deferred: Physical deletion was rejected because it breaks the current genesis-to-head proof; in-place plaintext rehashing was rejected because it hides history; a second compliance ledger was rejected because it duplicates truth; changing the established version-1 signed manifest was rejected for compatibility; unbounded JSON scans/exports were rejected for resource safety; Testcontainers, asynchronous delivery, source reconciliation, external anchoring, and KMS/HSM integration remain explicit pre-production work.
- Validation: A copied clean checkout passes `mvnw clean verify`: 40 tests, zero failures/errors/skips, 94.47% line coverage, and 76.32% branch coverage. The generated OpenAPI contract test confirms required paths, Basic/JWT schemes, and GET/POST-only audit-event operations. Documentation links, incremental commit authorship, private-repository evidence, container/CI configuration, and submission files were reviewed.
